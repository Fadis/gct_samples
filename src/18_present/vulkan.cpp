...
int main() {
  ...
  while( 1 ) {
    ...
    // コマンドバッファを実行
    {
      VkPipelineStageFlags dest_stage_mask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
      VkCommandBuffer command_buffer = **fb.command_buffer;
      VkSemaphore wait_semaphore = **sync.image_acquired;
      VkSemaphore signal_semaphore = **sync.draw_complete;
      ...
      submit_info.pWaitDstStageMask = &dest_stage_mask;
      // 終わったらこのセマフォに通知
    ...
    {
      VkSemaphore wait_semaphore = **sync.draw_complete;
      VkSwapchainKHR raw_swapchain = **swapchain;
