...
vec3 light(
  ...
  vec3 specular = walter( L, V, N, roughness, mix( vec3( 1, 1, 1 ), diffuse_color, metallicness ) );
  float diffuse = max( dot( L, N ), 0 ) /pi * burley( L, V, N, roughness );
  vec3 linear = ( max( dot( L, N ), 0 ) * ( max( specular, vec3( 0, 0, 0 ) ) + ( 1 - metallicness ) * diffuse * diffuse_color ) ) * light_energy + emissive + ambient * diffuse_color;
  linear = linear * 0.5;
  //return ;
  return linear;
}
...
vec3 light_with_mask(
  ...
  vec3 specular = walter( L, V, N, roughness, mix( vec3( 1, 1, 1 ), diffuse_color, metallicness ) );
  float diffuse = max( dot( L, N ), 0 ) /pi * burley( L, V, N, roughness );
  vec3 linear = mix( ( max( dot( L, N ), 0 ) * ( max( specular, vec3( 0, 0, 0 ) ) + ( 1 - metallicness ) * diffuse * diffuse_color ) + ambient * diffuse_color ) * light_energy + emissive, ambient * light_energy * diffuse_color + emissive, masked );
  return linear;
}
